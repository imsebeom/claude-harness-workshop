...
| 폴더/파일 | 설명 |
|----------|------|
| `CLAUDE.md` | 글로벌 지침 슬림판 (한국어, sonnet 고정, 완료 조건 합의, 컨텍스트 앵커) |
...
워크숍 참가자 대부분은 한컴 미설치자이므로 2차 경로를 쓰게 됩니다. 다음 한 줄로 pyhwp를 설치하세요.

```bash
pip install pyhwp
```

`.hwpx` 파일은 순수 파이썬 파서가 처리하므로 추가 설치가 필요 없습니다.
...
선택 메뉴가 뜨면 화살표로 **medium**을 고르고 `Enter`. `low`/`medium`/`high`/`xhigh`/`max` 다섯 단계 중 워크숍 실습은 `medium`으로 충분합니다. **기본은 `high`라서 그대로 두면 Pro 요금제 5시간 한도가 빠르게 깎입니다** — 빈 PC로 GitHub raw fetch 셋팅 한 번에 약 10%가 빠진 측정값이 있고, `medium`으로 낮추면 한 자릿수 % 안쪽으로 떨어집니다. `high`·`xhigh`·`max`는 복잡한 디버깅·아키텍처 설계 같은 곳에 한 번 쓰고 다시 내리는 도구로 두세요. 한 번 고른 단계는 그 세션 내내 유지됩니다.
