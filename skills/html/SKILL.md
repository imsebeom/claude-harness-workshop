...
같은 폴더(또는 인접한 `md/` 폴더)에 `<원본파일명>.md`가 이미 있으면 **그대로 재사용**한다 — `/md` 스킬이 같은 PDF에 대해 만들어 둔 결과가 있다면 다시 추출하지 않는다.
...
- `markitdown` 기반(`/md`와 동일한 엔진)이라 헤딩·표 구조가 일부 잡혀 본문 분할·번역에 LLM이 더 적게 일한다.
...
## 스크립트 참조
- `scripts/extract_md.py` — PDF → md (markitdown 기반, /md와 동일 엔진. 본문 분할·번역에 LLM 부담 ↓)
