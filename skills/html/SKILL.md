...
- 출력 폴더: `<your-project-dir>/<프로젝트명>/`

### 2. 본문 마크다운 추출 (md 우선 재사용)
같은 폴더(또는 인접한 `md/` 폴더)에 `<원본파일명>.md`가 이미 있으면 **그대로 재사용**한다 — `/rag` 스킬이 같은 PDF에 대해 만들어 둔 결과가 있다면 다시 추출하지 않는다.

없으면 다음 한 줄로 추출한다.

```bash
PYTHONIOENCODING=utf-8 python "~/.claude/skills/html/scripts/extract_md.py" <pdf_path> <project_dir>/<원본파일명>.md
```

- `markitdown` 기반(`/rag`와 동일한 엔진)이라 헤딩·표 구조가 일부 잡혀 본문 분할·번역에 LLM이 더 적게 일한다.
- 분량이 크면 (>100KB) 이후 HTML 조립을 **general-purpose 에이전트에 위임**해 본문 컨텍스트를 보호한다.
- 레거시 raw.txt 흐름이 필요하면 `scripts/extract_text.py`도 그대로 남아 있다.

### 3. 벡터 도형 추출
```bash
PYTHONIOENCODING=utf-8 python "~/.claude/skills/html/scripts/extract_figures.py" <pdf_path> <project_dir>/figs
```
...
### 4. HTML 조립
`scripts/html_shell.py`를 import하여 뼈대를 가져오거나, 직접 추출한 md를 파싱해 섹션(`<section class="chapter accent-*">`)에 배치한다.
...
- 에이전트에게 md 경로, figs 경로, 메타 정보, html_shell.py 경로를 전달
...
## 스크립트 참조
- `scripts/extract_md.py` — PDF → md (markitdown 기반, /rag와 동일 엔진. 본문 분할·번역에 LLM 부담 ↓)
- `scripts/extract_text.py` — PDF → raw.txt (페이지 구분자 포함, 레거시)
